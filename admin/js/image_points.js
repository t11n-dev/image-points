jQuery(document).ready(function($){		
	...
	$('body').on('click','[id*=meta-image-button]',function(e){
		...
		meta_image_frame.on('select', function(){
			...
			$('#image_points_main_image').val(media_attachment.url);
	...
	*/
  	...
  	$('.add_point').click(function(){  
  		...
  		var nonceForm = $('#image_points_meta_box_nonce').val();
  		...
  		$.ajax({
			...
			data : {
				action		:	"image_points_clone_point", 
  	...
  	function calc_custom_position(){
  		var $img = $('.pins_img');
  		if ($img.length && $img[0].naturalWidth === 0) {
  			$img.on('load', function(){ calc_custom_position(); });
  			return;
  		}
