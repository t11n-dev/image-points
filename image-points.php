...
define('IMAGE_POINTS_POINT_DEFAULT', json_encode(array(
	'countPoint'	=>	'',
	'content'		=>	'',
	'left'			=>	'',
	'top'			=>	'',
...
define('IMAGE_POINTS_PINS_DEFAULT', json_encode(array(
	'countPoint'	=>	'',
	'imgPoint'		=>	'',
	'top'			=>	'',
	'left'			=>	''
...
//include
include 'admin/inc/cpt-image-points.php';
include 'admin/inc/add-shortcode-image-points.php';
include 'admin/inc/metabox-donate.php';
include 'admin/inc/settings.php';

//load_textdomain('image-points', dirname(__FILE__) . '/languages/image-points-' . get_locale() . '.mo');
//load_plugin_textdomain( 'image-points', false, plugin_basename( dirname( __FILE__ ) ) . '/i18n/languages' );
		...
		<tbody>
			...
			<tr>
				...
				<td class="svl-input">
					<div class="pins-position-wrap">
						<p>
							<label><input type="radio" name="choose_type" value="center_center" <?php echo ($pins_more_option['position'] == 'center_center'?'checked="checked"':'')?>><?php esc_html_e('Center center','image-points')?></label>
							<label><input type="radio" name="choose_type" value="top_left" <?php echo ($pins_more_option['position'] == 'top_left'?'checked="checked"':'')?>><?php esc_html_e('Top Left','image-points')?></label>
							<label><input type="radio" name="choose_type" value="top_center" <?php echo ($pins_more_option['position'] == 'top_center'?'checked="checked"':'')?>><?php esc_html_e('Top Center','image-points')?></label>
							<label><input type="radio" name="choose_type" value="top_right" <?php echo ($pins_more_option['position'] == 'top_right'?'checked="checked"':'')?>><?php esc_html_e('Top Right','image-points')?></label>
							<label><input type="radio" name="choose_type" value="right_center" <?php echo ($pins_more_option['position'] == 'right_center'?'checked="checked"':'')?>><?php esc_html_e('Right Center','image-points')?></label>
							<label><input type="radio" name="choose_type" value="bottom_right" <?php echo ($pins_more_option['position'] == 'bottom_right'?'checked="checked"':'')?>><?php esc_html_e('Bottom Right','image-points')?></label>
							<label><input type="radio" name="choose_type" value="bottom_center" <?php echo ($pins_more_option['position'] == 'bottom_center'?'checked="checked"':'')?>><?php esc_html_e('Bottom Center','image-points')?></label>
							<label><input type="radio" name="choose_type" value="bottom_left" <?php echo ($pins_more_option['position'] == 'bottom_left'?'checked="checked"':'')?>><?php esc_html_e('Bottom Left','image-points')?></label>
							<label><input type="radio" name="choose_type" value="left_center" <?php echo ($pins_more_option['position'] == 'left_center'?'checked="checked"':'')?>><?php esc_html_e('Left Center','image-points')?></label>
							<label><input type="radio" name="choose_type" value="custom_center" <?php echo ($pins_more_option['position'] == 'custom_center'?'checked="checked"':'')?>><?php esc_html_e('Custom','image-points')?></label>
			...
			<tr>
				...
				<td class="svl-input">
					<div class="pins-position-wrap">
						<p>
							<label><input type="radio" name="pins_animation" value="none" <?php echo ($pins_more_option['pins_animation'] == 'none'?'checked="checked"':'')?>><?php esc_html_e('None','image-points')?></label>
							<label><input type="radio" name="pins_animation" value="pulse" <?php echo ($pins_more_option['pins_animation'] == 'pulse'?'checked="checked"':'')?>><?php esc_html_e('Pulse','image-points')?></label>
						</p>
					</div>
				</td>				
			</tr>
...
function image_points_is_base64($string) {
...
function image_points_save_meta_box_data( $post_id ) {
	...
	if(is_array($pointdata)){
		$dataPoints = image_points_convert_array_data($pointdata);
	}
	$data_post = array(
	...
	update_post_meta($post_id, 'image_points_content', json_encode($data_post));
	/*remove_action( 'save_post', 'image_points_save_meta_box_data' );
	wp_update_post(array(
		'ID'			=>	$post_id,
		'post_content'	=>	maybe_serialize(wp_unslash($data_post)),
		'post_type'		=>	'image_points'
	));	
...
function image_points_admin_script() {
	...
	if( $typenow == 'image_points' ) {
		...
		wp_localize_script( 'image_points', 'meta_image',
			array(
...
function image_points_get_input_point_default($data = array()){
	...
	ob_start();
	?>	
	<div class="image-points-popup list_points" tabindex="-1" role="dialog" id="info_draggable<?php echo intval($countPoint)?>" data-popup="info_draggable<?php echo intval($countPoint)?>" data-points="<?php echo intval($countPoint)?>">
			...
			<div class="image-points-popup-modal-content">
				<div class="image-points-popup-modal-header">
		  		...
		  		<div class="image-points-popup-modal-body">
					...
					<div class="image_points_row">
						<div class="image_points_col_3">
							...
							<input type="text" name="pointdata[linkpins][]" value="<?php echo esc_attr($pointLink)?>" placeholder="<?php esc_attr_e('Link to pins', 'image-points');?>"/>
							</label><br>
			  	...
			  	<div class="image-points-popup-modal-footer">
					...
					<button type="button" class="button button-primary button-large" data-popup-close="info_draggable<?php echo esc_attr($countPoint)?>"><?php esc_html_e('Done & Close','image-points')?></button>
			  	</div>
			</div><!-- /.modal-content -->
		</div><!-- /.modal-dialog -->
	</div><!-- /.modal -->		
...
function image_points_convert_array_data($inputArray = array()){
	$aOutput =  array();		
	$firstKey = null;
